...
# Debian publishes a security update before the base image is rebuilt around it,
# so a freshly built image can still carry a package whose fix has been
# available for days — pcre2 was three HIGH out-of-bounds writes behind
# (CVE-2026-86145, CVE-2026-89157, CVE-2026-89161) on an image built today.
# Upgrading every package that has a fix is the same predicate the scan gate
# uses, which keeps this a property of the image rather than a list of package
# names maintained by hand. The apt lists go in the same layer, so nothing here
# is shipped and the runtime still carries no package manager.
RUN apt-get update \
